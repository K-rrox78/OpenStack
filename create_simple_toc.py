...
for level, title in headings:
    ...
    if level == 'h2':
        # Indentation pour les titres de niveau 2
        ...
        # Indentation plus importante pour les titres de niveau 3
        p.paragraph_format.left_indent = Inches(0.5)
        run = p.add_run(f"   - {title}")
        run.font.size = Pt(11)
        run.font.color.rgb = RGBColor(0, 0, 100)  # Bleu foncé
...
print(f"Le document avec sommaire formaté a été créé: '{output_docx}'")
